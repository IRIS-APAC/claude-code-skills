...
After generating all agent responses for the round, let the user know they can speak naturally with the agents by typing their response, or use the `AskUserQuestion` tool to offer an exit option:

- **Question:** "Continue the conversation or exit Party Mode?"
- **Header:** "Action"
- **Options:**
  - **Continue chatting** — Keep the discussion going (you can also just type your next message)
  - **Exit Party Mode** — End the collaborative session
