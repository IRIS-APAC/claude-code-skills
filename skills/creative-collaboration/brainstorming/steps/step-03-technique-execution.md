...
---
advancedElicitationTask: ../advanced-elicitation/workflow.md
---
...
Use the `AskUserQuestion` tool to check continuation:

- **Question:** "What would be most valuable right now?"
- **Header:** "Direction"
- **Options:**
  - **Continue exploring** — Keep going with the current technique element
  - **Next element** — Move to the next technique element
  - **Different angle** — Take a different angle on the current element
  - **Most exciting idea** — Jump to the most exciting idea we've discovered so far

**Remember:** The user can also say **"next technique"** or **"move on"** at any time to immediately document current progress and start the next technique."
...
Use the `AskUserQuestion` tool to present next action options:

- **Question:** "What would you like to do next? (Unless we've hit 100+ ideas, I recommend keeping the exploration going!)"
- **Header:** "Next step"
- **Options:**
  - **Keep exploring** — We're just getting warmed up with this technique!
  - **Try different technique** — Fresh perspective on the same topic
  - **Go deeper (Elicitation)** — Develop a promising concept further using Advanced Elicitation
  - **Move to organization** — Only when you feel we've thoroughly explored

### 8. Handle Menu Selection

#### If "Move to organization":

- **Append the technique execution content to `{output_folder}/brainstorming/brainstorming-session-{{date}}.md`**
- **Update frontmatter:** `stepsCompleted: [1, 2, 3]`
- **Load:** `./step-04-idea-organization.md`

#### If "Keep exploring", "Try different technique", or "Go deeper (Elicitation)":

- **Stay in Step 3** and restart the facilitation loop for the chosen path.
- For "Go deeper (Elicitation)", invoke Advanced Elicitation: `{advancedElicitationTask}`
